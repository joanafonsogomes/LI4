(function ($) {
   ...
   // var new = document.currentScript.getAttribute('three');
    ...
    for (var d3 = [], i = 0; 10 >= i; i += 1) d3.push([i, parseInt(arr[i])]);
    var options = {
        series: {
            shadowSize: 0,
            lines: {
    ...
    $("#line-chart")[0] && $.plot($("#line-chart"), [{
        ...
        if (item) {
            ...
            $(".flot-tooltip").html(item.series.label + " Venda/Aluguer de " + y).css({
                top: item.pageY + 5,
                left: item.pageX + 5
            }).show()
